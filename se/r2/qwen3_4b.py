...
OUTPUT_DIR = "/home/shubhamdas-pg/tlr/se/r2/qwen3-4b"
...
DICT_TOKENS = [tokenize_sanskrit(entry["san"]) for entry in DICTIONARY]
BM25 = BM25Okapi(DICT_TOKENS)

def get_lexical_matches(san_text, c=2):
    query_tokens = tokenize_sanskrit(san_text)
    n = len(query_tokens)
    k = min(50, max(10, c * n))
...
def build_messages(san_text):
    matches = get_lexical_matches(san_text)
