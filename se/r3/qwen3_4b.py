...
OUTPUT_DIR = "/home/shubhamdas-pg/tlr/se/r3/qwen3-4b"
...
biencoder = SentenceTransformer(BI_ENCODER, device=DEVICE)
...
DICT_TEXTS = [entry["san"] for entry in DICTIONARY]
DICT_EMB = biencoder.encode(DICT_TEXTS, convert_to_tensor=True, normalize_embeddings=True)

def get_semantic_matches(san_text, c=2):
    query_emb = biencoder.encode(san_text, convert_to_tensor=True, normalize_embeddings=True)
    scores = util.cos_sim(query_emb, DICT_EMB)[0]
    n = len(san_text.split())
    k = min(50, max(10, c * n))
    top_indices = torch.topk(scores, k=k).indices.tolist()
    return [DICTIONARY[i] for i in top_indices]
...
def build_rag_context(matches):
    ...
    for entry in matches:
        context_lines.append(f'Sanskrit: {entry["san"]}, English: {entry.get("eng","")}')
    return "\n".join(context_lines)

def build_messages(san_text):
    matches = get_semantic_matches(san_text)
